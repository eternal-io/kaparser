...
impl<U: Slice, R: Read> Provider<U, R> {
...
impl<'i, T, R> Provider<&'i [T], R>
where
    T: Copy + PartialEq,
    R: Read,
{
    #[allow(clippy::should_implement_trait)]
    pub fn next<P, E>(&'i mut self, pat: &P) -> ProviderResult<P::Captured, E>
    where
        P: Pattern<&'i [T], E>,
        E: Situation,
    {
    ...
    pub fn peek<P, E>(&'i mut self, pat: &P) -> ProviderResult<P::Captured, E>
    where
        P: Pattern<&'i [T], E>,
        E: Situation,
    {
    ...
    R: Read,
{
    #[inline(always)]
    #[allow(unsafe_code)]
    fn pull<E: Situation>(&mut self, first_time: bool) -> ProviderResult<(&'i [T], bool), E> {
        match self {
            Source::Sliced { slice, consumed, .. } => {
                let _ = first_time;
                Ok((slice.split_at(*consumed).1, true))
    ...
    #[inline(always)]
    #[allow(unsafe_code)]
    fn bump(&'i mut self, n: usize) -> &'i [T] {
        match self {
            Source::Sliced { slice, consumed, .. } => {
                let left = slice[*consumed..]
                    .split_at_checked(n)
...
#[cfg(feature = "std")]
impl<U: Slice, R: Read> Source<U, R> {
    #[inline(always)]
